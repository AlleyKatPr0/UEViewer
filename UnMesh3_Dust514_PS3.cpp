...
namespace Dust514Ps3
{
	...
	)
	{
		...
		// pass this function the inner Edge delta block.
		if (DataSize < 1u + 4u)
		{
		...
		if (DataSize < Cursor + size_t(SeedCount) * 2u)
		{
		...
		size_t BitOffset = size_t(Cursor) * 8u;
		...
		for (uint32_t i = 8; i < IndexCount; ++i)
		{
			...
			else if (!ReadBitsHighFirst(Data, DataSize, BitOffset, BitsPerIndex, EncodedDelta))
			{
				SetError(OutError, "DecodeEdgeIndexBlock: ran out of bits while reading deltas.");
				return false;
			}

			const int32_t Delta = int32_t(EncodedDelta) - int32_t(DeltaOffset);
			const int32_t Restored = int32_t(Previous[i % 8u]) + Delta;

			if (Restored < 0 || uint32_t(Restored) >= VertexLimit)
			{
				SetError(OutError, "DecodeEdgeIndexBlock: restored index is outside vertex range.");
				return false;
			}
